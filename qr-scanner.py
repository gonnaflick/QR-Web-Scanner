...
if not cap.isOpened():
    pyautogui.alert('No se ha encontrado ninguna camara.',
                    'Advertencia', 'Aceptar')
    exit()
...
try:
    ...
    idNum = WebDriverWait(browser, 600).until(
        EC.presence_of_element_located((By.XPATH, xpath)))
except Exception as e:
    # Exception alert for timeout
    pyautogui.alert('Saliendo de la aplicacion por inactividad.',
                    'Advertencia', 'Aceptar')
    browser.close()
    exit()
...
while True:
    ...
    if not success:
        pyautogui.alert(
            'No se ha encontrado ninguna camara, saliendo de la aplicacion.', 'Advertencia', 'Aceptar')
    ...
    for barcode in decode(img):
        ...
        if oldData != newData:
            if time.time() - scanTime <= 3:
                ...
                draw(barcode, img,
                     f"Espere: {3 - (time.time() - scanTime)} seg.", 50, 50, 50)
                continue
            ...
            notify(f"Acceso registrado ID: {newData[1:-4].strip()}",
                   "./assets/audio/success.wav", "./assets/icon/success/96.png")
        ...
        # Else if, run the error for previous input
            ...
            # and delay is False, draw remove and send notification
                ...
                notify(f"Retire del escaner ID: {newData[1:-4].strip()}",
                       "./assets/audio/remove.wav", "./assets/icon/remove/96.png")
        ...
        # Else, run the error
            ...
            # Else draw error and send notification
                ...
                notify(f"Error desconocido detectado. Favor de reportarlo.",
                       "./assets/audio/error.wav", "./assets/icon/error/96.png")

                # Show dialog window to check user input for reset
                reset = pyautogui.confirm('Ha ocurrido un error desconocido. ¿Desea reiniciar la aplicación para intentar resolver el error?', "Advertencia", [
                                          'Reiniciar', 'Cancelar'])
                ...
                if reset == "Reiniciar":
                    ...
                    if not cap.isOpened():
                        pyautogui.alert(
                            'No se ha encontrado ninguna camara.', 'Advertencia', 'Aceptar')
                        exit()
                    cap.set(3, 300)
                    cap.set(4, 300)
                    oldData = ""
